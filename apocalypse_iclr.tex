...
that price is no guide to provider choice: cross-provider price spreads reach \maxpricex{} and
latency spreads \maxlatx{} for the identical model, ``the same model'' ranges from 35\% to 90\%
accuracy on hard reasoning, and the within-cell rank correlation between price and accuracy is
negligible (median Spearman $\corrqual$). Price is likewise an unreliable predictor of latency
and reliability---near zero on average with signs that flip across cells (median
$\corrlat$ and $\corrrel$). Consequently neither price nor the advertised quantization label
...
providers, price is not a reliable guide to the quality, latency, or reliability of the same
model, so a provider cannot be chosen from the price list---it must be measured.} The
...
and quantify that price does not predict quality (median within-cell Spearman $\corrqual$ over
\corrqualn{} cells with quality variance) and predicts latency/reliability no more consistently
...
Table~\ref{tab:main} reports all \numcells{} cells; Figure~\ref{fig:scatter} visualizes the
core claim. Three patterns hold.

\begin{figure}[t]\centering
\includegraphics[width=0.62\textwidth]{fig_price_quality.pdf}
\caption{Every point is a provider serving a given model on a given task; the $x$-axis is its
price relative to the cheapest provider for that same (model, task). Higher price does not yield
higher accuracy---the cloud is flat (and many cells saturate at $100\%$ regardless of price).}
\label{fig:scatter}
\end{figure}
...
\paragraph{(3) Price is no guide; only measurement orders providers.} Two providers serving the
same ``fp8'' model differ by 30 accuracy points; a bf16 endpoint can be the most expensive and
among the least accurate. We quantify this with the within-cell Spearman rank correlation
between provider price and each metric (Table~\ref{tab:corr}). Price and accuracy are
essentially uncorrelated (median $\corrqual$ over the \corrqualn{} cells with quality variance;
$|r|>0.5$ in only one). Price predicts latency and reliability no more usefully: the medians are
small ($\corrlat$, $\corrrel$) but, crucially, the \emph{sign flips across cells}, so no
global ``cheaper-is-faster'' or ``dearer-is-safer'' rule holds. (The reliability correlation is
additionally confounded by self-induced rate limits, Section~\ref{sec:meas}.) The actionable
claim is therefore not statistical independence but that \emph{no price-based heuristic reliably
selects the best provider on any axis}---measurement is required.

\begin{table}[t]\centering\small
\caption{Within-(model$\times$task) Spearman rank correlation of provider \emph{price} against
each metric, median over cells with variance. Near-zero medians and sign-flipping across cells
mean price is not a usable predictor.}
\label{tab:corr}
\begin{tabular}{lcc}
\toprule
Price vs. & Median Spearman & Interpretation \\
\midrule
Accuracy     & $\corrqual$ & paying more does not buy quality \\
Latency      & $\corrlat$  & no consistent price--speed relation \\
Reliability  & $\corrrel$  & no consistent price--uptime relation \\
\bottomrule
\end{tabular}
\end{table}
...
best \emph{and} whose availability exceeds a threshold (we use a $5$-point floor and $90\%$
availability). Table~\ref{tab:baselines} compares this policy against four baselines across all
\numcells{} cells: \emph{single-best} (always the most accurate provider), \emph{premium}
(always the most expensive---the ``you get what you pay for'' buyer), \emph{random}, and
\emph{cheapest} (sort by price). Our router attains $94\%$ mean accuracy---within one point of
single-best ($95\%$)---at $1.57\times$ the cheapest price, while the premium policy pays
$3.67\times$ for \emph{lower} accuracy and a $17\%$ below-floor rate. The price-blind policy is
cheapest ($1.0\times$) but is sub-floor in $28\%$ of cells. Only our policy combines near-best
quality, near-cheapest cost, the highest availability, and a $0\%$ below-floor rate; it
Pareto-dominates premium, random, and cheapest. Exposed as an OpenAI-compatible endpoint, the
policy lets a caller change one line (\texttt{base\_url}) and tag the task to obtain the per-task
floor; a savings ledger records the counterfactual cost. On capable models where quality
saturates, the router instead trades a little price for reliability, avoiding
cheap-but-rate-limited endpoints.

\begin{table}[t]\centering\small
\caption{Routing policy comparison over all \numcells{} (model$\times$task) cells. Rel.\ cost
normalizes each cell to its cheapest provider ($=1.0$). Below-floor is the fraction of cells
whose pick falls below the cell's quality floor. $^\ast$averaged per cell.}
\label{tab:baselines}
\input{baselines_table}
\end{table}
